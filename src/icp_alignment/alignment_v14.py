...
# def create_sphere_at_point(point_cloud, color, radius=0.1, resolution=20, point_count=1000):
#     center = np.asarray(point_cloud.points)[0]
...
#     # Convertir la esfera en una nube de puntos
#     sphere_cloud = sphere.sample_points_uniformly(number_of_points=point_count)
...
def icp_alignment(csv_files, folder_path, vis_map):
    ...
    if CONFIG["trajectory_sphere"]:
        trajectory_transformed_cloud = generate_sphere(radius=0.05, point_count=100) 
...
def save_map(global_map, timestamp):
    ...
    output_path = os.path.join(folder_path_map, f"{timestamp}_map.ply")
...
def save_trajectory(global_trajectory, timestamp):
    ...
    output_path = os.path.join(folder_path_trajectory, f"{timestamp}_trajectory.ply")
...
def save_person_cloud(person_global_cloud, timestamp):
    ...
    output_path = os.path.join(folder_path_trajectory, f"{timestamp}_person.ply")

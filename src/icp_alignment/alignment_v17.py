...
# v17 - Changed YOLO to be used locally to make it faster
...
def initialize_roboflow():
    # Initialize Roboflow
    model = YOLO("C:/Users/Albert/Desktop/lidar-fused-3d-map/YOLO_file/yolov8s.pt")
    return model
...
def yolo_add_boxes(image_file, output_image_path, model):
    ...
    # Run inference
    results = model(image, conf=0.10)[0]  # Ultralytics returns a list of Results objects

    # Convert predictions to the format you're using
...
def icp_alignment(csv_files, folder_path, vis_map):
    ...
    #  TODO
    base_dir = f"D:/LiDAR-captures/{sel}/bev_images_local"
    ...
    create_bev_image(
        ...
        res=0.02, x_range=(-5,5), y_range=(-5,5), z_range=(-2,2)
    )

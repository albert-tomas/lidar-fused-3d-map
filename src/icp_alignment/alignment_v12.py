...
def icp_alignment(csv_files, folder_path, vis_map, vis_trajectory):
    ...
    aux_cloud = o3d.geometry.PointCloud(person_cloud)
    aux_cloud += person_cloud
    
    global_map = input_cloud.voxel_down_sample(voxel_size)
    change_colors(global_map, COLORS["GREY"])
    global_map += person_cloud
    ...
    if CONFIG["print_realtime"]:
        display_point_cloud(vis_map, global_map, point_size=1.0)
        display_point_cloud(vis_trajectory, trajectory_global_cloud, point_size=5.0)
    ...
    for i in range(1, min(len(csv_files), max_number_of_clouds)):
        ...
        # filtered_cloud_highlighted = highliht_points_in_boxes(filtered_cloud, predictions, COLORS["GREEN"], res=0.02, x_range=(-5, 5), y_range=(-5, 5))
        
        # color_stats = count_points_by_color(filtered_cloud_highlighted,
        #     {"grey":  COLORS["GREY"],
        #     "green": COLORS["GREEN"]})
        ...
        global_map += filtered_cloud + person_cloud
        #TODO
        aux_cloud += person_cloud
        # global_map = highliht_points_in_boxes(global_map, predictions, COLORS["GREEN"], res=0.02, x_range=(-5, 5), y_range=(-5, 5))
        
        color_stats = count_points_by_color(global_map,
            {"grey":  COLORS["GREY"],
            "green": COLORS["GREEN"]})
    ...
    return global_map, trajectory_global_cloud, aux_cloud
...
def main():
    ...
    if CONFIG["print_realtime"]:
        ...
        vis_trajectory = setup_visualizer()
    else:
        vis_map = None
        vis_trajectory = None
    global_map, global_trajectory, aux_cloud = icp_alignment(csv_files, folder_path, vis_map, vis_trajectory)
    ...
    display_point_cloud(vis_map2, aux_cloud, point_size=2.0)
    vis_map2.run()

...
def icp_alignment(csv_files, folder_path, vis_map, vis_trajectory):
    ...
    input_cloud = highliht_points_in_boxes(input_cloud, predictions, COLORS["GREEN"], res=0.02, x_range=(-5, 5), y_range=(-5, 5))
    ...
    for i in range(1, min(len(csv_files), max_number_of_clouds)):
        ...
        color_stats = count_points_by_color(current_cloud_downsampled,
            {"grey":  COLORS["GREY"],
            "green": COLORS["GREEN"]})
    ...
    return global_map, trajectory_global_cloud
...
def main():
    ...
    global_map, global_trajectory = icp_alignment(csv_files, folder_path, vis_map, vis_trajectory)

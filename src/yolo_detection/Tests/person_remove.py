...
def create_bev_image(csv_file, output_image_path, res=0.01, x_range=(-3, 5), y_range=(-6, 5), z_range=(-2, 2)):
    # Load data from CSV
...
def remove_points_in_boxes(csv_path, predictions, output_csv_path, res=0.02, x_range=(-10, 10), y_range=(-10, 10)):
    # Load the original CSV
    df = pd.read_csv(csv_path)

    for pred in predictions['predictions']:
        # Box coordinates in pixels
        x1_px = int(pred['x'] - pred['width'] / 2)
        x2_px = int(pred['x'] + pred['width'] / 2)
        y1_px = int(pred['y'] - pred['height'] / 2)
        y2_px = int(pred['y'] + pred['height'] / 2)
